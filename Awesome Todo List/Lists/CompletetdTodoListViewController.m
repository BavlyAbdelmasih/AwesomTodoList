//
//  CompletetdTodoListViewController.m
//  Awesome Todo List
//
//  Created by iambavly on 2/27/21.
//

#import "CompletetdTodoListViewController.h"
#import "TodoListManager.h"
#import "DetailsViewController.h"

#import "TodoItem.h"

@interface CompletetdTodoListViewController ()
@property (weak, nonatomic) IBOutlet UITableView *tableView;

@end

@implementation CompletetdTodoListViewController{
...
-(void)fetchData{
[manager getTodoList];
arrayTableData = manager.getCompletetdTodoList;
[_tableView reloadData];
...
@try
 {
     ...
     if([searchText length] > 0){
         searchResults = [manager searchArrayWithString:searchText targetArray:manager.getCompletetdTodoList];
...
title:@"Done" handler:^(UIContextualAction * _Nonnull action, __kindof UIView * _Nonnull sourceView, void (^ _Nonnull completionHandler)(BOOL)) {
    ...
    TodoItem *item = [self->manager.getCompletetdTodoList objectAtIndex:indexPath.row];
 ...
 done.backgroundColor = [UIColor systemYellowColor]; //arbitrary color
